...
source "qemu" "boot-menu" {
  ...
  output_directory = "${path.root}/artifacts/boot-menu"
  vm_name          = "boot-menu"
  format           = "qcow2"
  disk_size        = 10240
  ...
  boot_command = [
    ...
    "e<wait2>",
    "<down><down><end>",
    " inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/ks-boot-menu.cfg net.ifnames=0 biosdevname=0 console=ttyS0,115200n8",
...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # Configure GRUB — extended timeout so boot menu is visible
      ...
      "echo 'GRUB_CMDLINE_LINUX=\"console=ttyS0,115200n8 console=tty0\"' | sudo tee -a /etc/default/grub",
  ...
  provisioner "shell" {
    script = "${path.root}/scripts/configure-emergency-shell.sh"
  }
}
